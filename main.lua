	...
	vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				...
				loadstring(game:HttpGet('https://raw.githubusercontent.com/Subbico/CatV5/main/init.lua'), 'init.lua')()
			]]
...
if not shared.VapeIndependent then
	...
	if isfile('catrewrite/games/'..game.PlaceId..'.lua') and shared.VapeDeveloper then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/Subbico/CatV5/'..readfile('catrewrite/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true)
			end)
