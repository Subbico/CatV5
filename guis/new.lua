...
local usedLanguage = isfolder('newcatvape/profiles') and isfile('newcatvape/profiles/language.txt') and readfile('newcatvape/profiles/language.txt') or 'Original'
...
local getText = function(language, text)
	return game:HttpGet('https://raw.githubusercontent.com/Subbico/CatV5/'..readfile('newcatvape/profiles/commit.txt')..'/'.. 'translations/'.. language.. '/'.. text .. '.txt')
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		...
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/Subbico/CatV5/'..readfile('newcatvape/profiles/commit.txt')..'/'..select(1, path:gsub('newcatvape/', '')), true)
		end)
...
guipane:CreateDropdown({
	...
	Function = function(val, mouse)
		if mouse then
			writefile('newcatvape/profiles/language.txt', val)
			shared.vapereload = true
			loadfile('newcatvape/init.lua')({
				Developer = getgenv().catvapedev
			})
